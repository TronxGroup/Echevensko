...
<script>
function addAriaSelected6988454000000575779 (){ var optionElem = event.target; var previousSelectedOption = optionElem.querySelector ( '[aria-selected=true]' ); if( previousSelectedOption ){ previousSelectedOption.removeAttribute ( 'aria-selected' ); } optionElem.querySelectorAll ( 'option' )[ optionElem.selectedIndex ].ariaSelected = 'true'; }
/* Do not remove this code. */
function rccallback6988454000000575779 (){ if( document.getElementById( 'recap6988454000000575779' ) != undefined ){ document.getElementById ( 'recap6988454000000575779' ).setAttribute ( 'captcha-verified', true ); } if( document.getElementById( 'recapErr6988454000000575779' ) != undefined && document.getElementById( 'recapErr6988454000000575779' ).style.visibility == 'visible' ){ document.getElementById ( 'recapErr6988454000000575779' ).style.visibility = 'hidden'; } }
function reCaptchaAlert6988454000000575779 (){ var recap = document.getElementById ( 'recap6988454000000575779' ); if( recap != undefined && recap.getAttribute( 'captcha-verified' ) == 'false' ){ document.getElementById ( 'recapErr6988454000000575779' ).style.visibility = 'visible'; return false; } return true; }
function validateEmail6988454000000575779 (){ var form = document.forms[ 'WebToLeads6988454000000575779' ]; var emailFld = form.querySelectorAll ( '[ftype=email]' ); var i; for( i = 0; i < emailFld.length; i++ ){ var emailVal = emailFld[ i ].value; if((emailVal.replace( /^\\s+|\\s+$/g,'' )).length != 0 ){ var atpos = emailVal.indexOf ( '@' ); var dotpos = emailVal.lastIndexOf ( '.' ); if( atpos < 1 || dotpos < atpos + 2 || dotpos + 2 >= emailVal.length ){ alert ( 'Introduzca una dirección de correo electrónico válida. ' ); emailFld[ i ].focus (); return false; } } } return true; }
function validateDateFormat6988454000000575779 (){ var form = document.forms[ 'WebToLeads6988454000000575779' ]; var dateFlds = form.querySelectorAll ( '[ftype=date]' ); var i; for( i = 0; i < dateFlds.length; i++ ){ var dateFld = dateFlds[ i ]; var usrPtrn = dateFld.placeholder; var dateVal = dateFld.value; if( dateVal.trim() != '' ){ var vald = dateFormatConvert.validate ( dateVal, usrPtrn ); if( !vald ){ alert ( 'Escriba una fecha válida ' ); dateFld.focus (); return false; } } } return true; }
function checkMandatory6988454000000575779 (){ var mndFileds = new Array ( 'Company', 'First Name', 'Last Name', 'Email', 'City', 'LEADCF3' ); var fldLangVal = new Array ( 'Empresa', 'Nombre', 'Apellido', 'Correo\\x20electr\\xF3nico', 'Ciudad', 'Mensaje' ); for( i = 0; i < mndFileds.length; i++ ){ var fieldObj = document.forms[ 'WebToLeads6988454000000575779' ][ mndFileds[ i ]]; if( fieldObj ){ if(((fieldObj.value ).replace( /^\\s+|\\s+$/g,'' )).length == 0 ){ if( fieldObj.type == 'file' ){ alert ( 'Seleccione un archivo para cargar.' ); fieldObj.focus (); return false; } alert ( fldLangVal[ i ] + ' no puede estar vacío.' ); fieldObj.focus (); return false; } else if( fieldObj.nodeName == 'SELECT' ){ if( fieldObj.options[fieldObj.selectedIndex].value == '-None-' ){ alert ( fldLangVal[ i ] + ' no puede ser nulo.' ); fieldObj.focus (); return false; } } else if( fieldObj.type == 'checkbox' ){ if( fieldObj.checked == false ){ alert ( 'Please accept ' + fldLangVal[ i ]); fieldObj.focus (); return false; } } try { if( fieldObj.name == 'Last Name' ){ name = fieldObj.value; } } catch ( e ){} } } if( !validateEmail6988454000000575779()){ return false; } if( !reCaptchaAlert6988454000000575779()){ return false; } if( !validateDateFormat6988454000000575779()){ return false; } var urlparams = new URLSearchParams ( window.location.search ); if( urlparams.has( 'service' ) && ( urlparams.get( 'service' ) === 'smarturl' )){ var webform = document.getElementById ( 'webform6988454000000575779' ); var service = urlparams.get ( 'service' ); var smarturlfield = document.createElement ( 'input' ); smarturlfield.setAttribute ( 'type', 'hidden' ); smarturlfield.setAttribute ( 'value', service ); smarturlfield.setAttribute ( 'name', 'service' ); webform.appendChild ( smarturlfield ); }
document.querySelector ( '.crmWebToEntityForm .formsubmit' ).setAttribute ( 'disabled', true ); }
function tooltipShow6988454000000575779 ( el ){ var tooltip = el.nextElementSibling; var tooltipDisplay = tooltip.style.display; if( tooltipDisplay == 'none' ){ var allTooltip = document.getElementsByClassName ( 'zcwf_tooltip_over' ); for( i = 0; i < allTooltip.length; i++ ){ allTooltip[ i ].style.display = 'none'; } tooltip.style.display = 'block'; } else { tooltip.style.display = 'none'; } }
</script>
<!-- Do not remove this --- Analytics Tracking code starts -->
<script id = 'wf_anal' src = 'https://crm.zohopublic.com/crm/WebFormAnalyticsServeServlet?rid=&tw=73e511fcc5d38b1ec0d0529d3485dba39bd01d271136baa72229b93853edf71b'> </script>
...
export default function ZohoFormEmbed() {
  ...
  useEffect(() => {
    ...
    // Inyecta el HTML
    el.innerHTML = ZOHO_HTML
      // por si en el futuro agregas texto/links de privacidad en el bloque:
      .replaceAll('href="/privacidad"', 'href="/privacidad-condiciones"')
      .replaceAll('href="/terminos"', 'href="/privacidad-condiciones"');

    // Re-ejecuta todos los <script> (externos e inline)
  ...
  return <div ref={ref} suppressHydrationWarning className="w-full" />;
}
